...
public class CarouselPagerAdapter1 extends FragmentPagerAdapter implements ViewPager.OnPageChangeListener {
    ...
    private News_feed2Activity context;
    private FragmentManager fragmentManager;
    private float scale;

    public CarouselPagerAdapter1(News_feed2Activity context, FragmentManager fm) {
        super(fm);
    ...
    public Fragment getItem(int position) {
        ...
        try {
            if (position == News_feed2Activity.FIRST_PAGE)
                scale = BIG_SCALE;
            else
                scale = SMALL_SCALE;

            position = position % News_feed2Activity.count;
    ...
    public int getCount() {
        int count = 0;
        try {
            count = News_feed2Activity.count * News_feed2Activity.LOOPS;
        } catch (Exception e) {
